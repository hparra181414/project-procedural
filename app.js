...
function deleteString(entryIndex, stringId) {
  ...
  entry.strings = entry.strings.filter((s) => s.id !== stringId);
...
function updateString(entryIndex, stringId, timeSeconds, inputs) {
  ...
  const oldString = entry.strings.find((s) => s.id === stringId);
  const updated = buildStringResult(drill, timeSeconds, inputs);
  updated.id = stringId; // keep the same id
  updated.loggedAt = oldString ? oldString.loggedAt : updated.loggedAt; // preserve original log time

  entry.strings = entry.strings.map((s) => (s.id === stringId ? updated : s));
...
function renderApp() {
  ...
  container.innerHTML = session.drillEntries.map((entry, index) => {
    ...
    const stringsHtml = entry.strings.map((s) => {
      ...
      const menuOpen = openMenuStringId === s.id;
    ...
    const editingString = (editingContext && editingContext.entryIndex === index)
      ? entry.strings.find((s) => s.id === editingContext.stringId)
      : null;
    ...
    return `
      <div class="drill-entry" data-index="${index}">
        ...
        <div class="mini-form">
          ${drill.parTime ? `<p class="par-hint">Par: ${drill.parTime}s</p>` : ''}
          ${buildMiniForm(drill, index, editingString)}
        </div>
      </div>
    `;
  }).join('');
