...
let currentView = 'main'; // 'main', 'history', 'prs', or 'historyDetail'
...
function renderTemplateDrillOptions() {
  document.getElementById('template-drill-options').innerHTML =
    getDrills().map((d) => `<option value="${d.name}">`).join('');
}
...
function addDrillToTemplateSetup() {
  const input = document.getElementById('template-add-drill-input');
  const name = input.value.trim();
  if (!name) return;

  const drill = getDrills().find((d) => d.name.toLowerCase() === name.toLowerCase());
  if (!drill) {
    alert('This drill isn\'t in your Drill Library yet - add it via a session first, then it can go in a template.');
    return;
  }
  if (pendingTemplateDrillIds.includes(drill.id)) {
    input.value = '';
    return;
  }

  pendingTemplateDrillIds.push(drill.id);
  input.value = '';
  renderTemplateDrillList();
}
