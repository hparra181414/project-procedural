...
// ---- Metronome (reuses the same beep engine as the shot timer) ----
...
function startMetronome() {
  const bpm = parseInt(document.getElementById('metronome-bpm').value) || 60;
  const intervalMs = 60000 / bpm;
  playDryFireBeep();
  metronomeIntervalId = setInterval(playDryFireBeep, intervalMs);
...
// ---- Trained-date marker ----
// The ONLY thing Dry Fire ever saves: today's date, so the Shooter's Log
// calendar can show a "Trained" dot. No reps, times, or results attached.
function markDryFireTrained() {
  const today = new Date().toISOString().split('T')[0];
  const dates = getDryFireTrainedDates();
  if (!dates.includes(today)) {
    dates.push(today);
    localStorage.setItem('dryFireTrainedDates', JSON.stringify(dates));
  }
}
function getDryFireTrainedDates() {
  const raw = localStorage.getItem('dryFireTrainedDates');
  return raw ? JSON.parse(raw) : [];
}
...
function exportData() {
  const data = {
    ...
    matches: getMatches(),
    dryFireTrainedDates: getDryFireTrainedDates(),
    officialClassification: getOfficialClassification(),
...
function importData(file) {
  ...
  reader.onload = (e) => {
    ...
    if (data.dryFireTrainedDates) localStorage.setItem('dryFireTrainedDates', JSON.stringify(data.dryFireTrainedDates));
    if (data.officialClassification) saveOfficialClassification(data.officialClassification.class, data.officialClassification.percent);
...
// Unified calendar (lives in Shooter's Log) - shows training days, dry-fire
// "Trained" days, and completed/upcoming matches together as small dots per day,
// since a single day can have more than one type of activity.
function renderUnifiedCalendar() {
  const container = document.getElementById('unified-calendar');
  if (!container) return;

  const trainingDates = new Set(getSessions().map((s) => s.date));
  const dryFireDates = new Set(getDryFireTrainedDates());
  const matches = getMatches();
  const completedMatchDates = new Set(matches.filter((m) => m.status === 'completed').map((m) => m.date));
  const upcomingMatchDates = new Set(matches.filter((m) => m.status === 'upcoming').map((m) => m.date));
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    let dots = '';
    if (trainingDates.has(dateStr)) dots += '<span class="cal-dot cal-dot-training"></span>';
    if (dryFireDates.has(dateStr)) dots += '<span class="cal-dot cal-dot-dryfire"></span>';
    if (completedMatchDates.has(dateStr)) dots += '<span class="cal-dot cal-dot-match-completed"></span>';
    if (upcomingMatchDates.has(dateStr)) dots += '<span class="cal-dot cal-dot-match-upcoming"></span>';
    cells += `<div class="cal-cell-multi" data-date="${dateStr}"><span class="cal-day-num">${d}</span><span class="cal-dots">${dots}</span></div>`;
  }

  container.innerHTML = `
    <div class="cal-header">
      <button id="unified-cal-prev-btn" class="secondary">&lsaquo;</button>
      <span>${monthLabel}</span>
      <button id="unified-cal-next-btn" class="secondary">&rsaquo;</button>
    </div>
    <div class="cal-weekdays"><span>S</span><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span></div>
    <div class="cal-grid">${cells}</div>
    <div class="cal-legend-multi">
      <span><span class="cal-dot cal-dot-training"></span> Training</span>
      <span><span class="cal-dot cal-dot-dryfire"></span> Trained</span>
      <span><span class="cal-dot cal-dot-match-completed"></span> Match</span>
      <span><span class="cal-dot cal-dot-match-upcoming"></span> Upcoming</span>
    </div>
  `;
}
...
function toggleMatchResultsFieldsVisibility() {
  ...
  document.getElementById('match-results-fields').classList.toggle('hidden', status !== 'completed');
}
...
// ============================================================
// SHOOTER'S LOG (Potential / Realized, third peer to Training and Match Tracking)
// ============================================================

// Compares each drill's first-ever attempt to its current best on the same
// metric, and surfaces whichever drill improved the most (by percentage,
// so drills with different scoring types can be compared fairly).
function getMostImprovedDrill() {
  let best = null;

  getDrills().forEach((drill) => {
    const strings = getAllStringsForDrill(drill.id); // chronological, oldest first
    if (strings.length < 2) return;

    const first = strings[0];
    let firstVal, bestVal, improvementPct;

    if (drill.scoringType === 'uspsa_zones') {
      firstVal = first.hitFactor;
      bestVal = Math.max(...strings.map((s) => s.hitFactor));
      improvementPct = firstVal > 0 ? ((bestVal - firstVal) / firstVal) * 100 : null;
    } else if (drill.scoringType === 'hit_miss') {
      firstVal = first.rate;
      bestVal = Math.max(...strings.map((s) => s.rate));
      improvementPct = firstVal > 0 ? ((bestVal - firstVal) / firstVal) * 100 : null;
    } else {
      firstVal = first.timeSeconds; // time_only: lower is better
      bestVal = Math.min(...strings.map((s) => s.timeSeconds));
      improvementPct = firstVal > 0 ? ((firstVal - bestVal) / firstVal) * 100 : null;
    }

    if (improvementPct !== null && improvementPct > 0 && (!best || improvementPct > best.improvementPct)) {
      best = { drillName: drill.name, improvementPct: Math.round(improvementPct * 10) / 10 };
    }
  });
...
// Rolling 30-day window, not an all-time total, so it reflects current training load.
function getTrainingVolumeLast30Days() {
  const cutoff = new Date();
  cutoff.setDate(cutoff.getDate() - 30);
  const recentSessions = getSessions().filter((s) => new Date(s.date) >= cutoff);
  const roundsFired = recentSessions.reduce((sum, s) => sum + getSessionRoundsFired(s), 0);
  return { sessionCount: recentSessions.length, roundsFired };
}

function renderPotential() {
  const improved = getMostImprovedDrill();
  const volume = getTrainingVolumeLast30Days();

  const improvedRow = improved
    ? `<div class="detail-row"><span class="detail-label">Most Improved Drill</span><span class="detail-value">${improved.drillName} (+${improved.improvementPct}%)</span></div>`
    : `<p class="empty-state">Log at least 2 attempts on a drill to see improvement.</p>`;

  document.getElementById('potential-content').innerHTML = `
    <div class="detail-about">
      ${improvedRow}
      <div class="detail-row"><span class="detail-label">Sessions (30 days)</span><span class="detail-value">${volume.sessionCount}</span></div>
      <div class="detail-row"><span class="detail-label">Rounds Fired (30 days)</span><span class="detail-value">${volume.roundsFired}</span></div>
    </div>
  `;
}

function getOfficialClassification() {
  return {
    class: localStorage.getItem('officialClassificationClass') || '',
    percent: localStorage.getItem('officialClassificationPercent') || '',
  };
}
function saveOfficialClassification(cls, percent) {
  localStorage.setItem('officialClassificationClass', cls);
  localStorage.setItem('officialClassificationPercent', percent);
}

function openClassificationEdit() {
  const cls = getOfficialClassification();
  document.getElementById('classification-class-input').value = cls.class;
  document.getElementById('classification-percent-input').value = cls.percent;
  document.getElementById('classification-edit-form').classList.remove('hidden');
}
function closeClassificationEdit() {
  document.getElementById('classification-edit-form').classList.add('hidden');
}
function saveClassificationEdit() {
  const cls = document.getElementById('classification-class-input').value.trim();
  const percent = document.getElementById('classification-percent-input').value.trim();
  saveOfficialClassification(cls, percent);
  closeClassificationEdit();
  renderRealized();
}

// Rollup of match performance for the Realized section - distinct from the
// per-match Performance Tracking chart in Match Tracking, this summarizes
// standing at a glance (best/worst, and a simple recent-vs-earlier trend).
function getMatchRollup() {
  const completed = [...getMatches()]
    .filter((m) => m.status === 'completed' && m.overallPercent != null)
    .sort((a, b) => new Date(a.date) - new Date(b.date));
  ...
  const percents = completed.map((m) => m.overallPercent);
  ...
  if (completed.length >= 2) {
    ...
    const recentAvg = completed.slice(-recentCount).reduce((s, m) => s + m.overallPercent, 0) / recentCount;
    const earlierCount = Math.max(1, completed.length - recentCount);
    const earlierAvg = completed.slice(0, earlierCount).reduce((s, m) => s + m.overallPercent, 0) / earlierCount;
    trend = Math.round((recentAvg - earlierAvg) * 10) / 10;
  }

  return { best, worst, trend, count: completed.length };
}

function renderRealized() {
  const cls = getOfficialClassification();
  const rollup = getMatchRollup();

  const rollupHtml = rollup
    ? `
      <div class="detail-row"><span class="detail-label">Matches Logged</span><span class="detail-value">${rollup.count}</span></div>
      <div class="detail-row"><span class="detail-label">Best Overall %</span><span class="detail-value">${rollup.best}%</span></div>
      <div class="detail-row"><span class="detail-label">Worst Overall %</span><span class="detail-value">${rollup.worst}%</span></div>
      ${rollup.trend !== null ? `<div class="detail-row"><span class="detail-label">Recent Trend</span><span class="detail-value">${rollup.trend >= 0 ? '+' : ''}${rollup.trend}%</span></div>` : ''}
    `
    : `<p class="empty-state">Log Overall % on completed matches to see your rollup.</p>`;

  document.getElementById('realized-content').innerHTML = `
    <div class="detail-about">
      <div class="detail-row"><span class="detail-label">Official Classification</span><span class="detail-value">${cls.class || '-'}${cls.percent ? ` (${cls.percent}%)` : ''}</span></div>
      ${rollupHtml}
    </div>
  `;
}

function goToShooterLog() {
  currentView = 'shooterLog';
  renderApp();
}

function renderShooterLogView() {
...
const ALL_VIEW_IDS = [
  ...
  'dry-fire-view', 'match-tracking-view', 'shooter-log-view',
];
...
function renderApp() {
  ...
  if (currentView === 'shooterLog') {
    stopSessionTimer();
    document.getElementById('shooter-log-view').classList.remove('hidden');
    renderShooterLogView();
    return;
  }
...
document.getElementById('view-unified-calendar-btn').addEventListener('click', goToShooterLog);
...
document.getElementById('goto-shooter-log-btn').addEventListener('click', goToShooterLog);
document.getElementById('back-from-shooter-log-btn').addEventListener('click', goHome);
document.getElementById('edit-classification-btn').addEventListener('click', openClassificationEdit);
document.getElementById('save-classification-btn').addEventListener('click', saveClassificationEdit);
document.getElementById('cancel-classification-btn').addEventListener('click', closeClassificationEdit);
document.getElementById('metronome-toggle-btn').addEventListener('click', toggleMetronome);

document.getElementById('unified-calendar').addEventListener('click', (e) => {
  if (e.target.id === 'unified-cal-prev-btn') {
    calendarViewMonth--;
    if (calendarViewMonth < 0) { calendarViewMonth = 11; calendarViewYear--; }
    renderUnifiedCalendar();
    return;
  }
  if (e.target.id === 'unified-cal-next-btn') {
    calendarViewMonth++;
    if (calendarViewMonth > 11) { calendarViewMonth = 0; calendarViewYear++; }
    renderUnifiedCalendar();
    return;
  }
  const cell = e.target.closest('.cal-cell-multi[data-date]');
  if (!cell) return;
  const date = cell.dataset.date;

  const match = getMatches().find((m) => m.date === date);
  if (match) {
    currentView = 'matchTracking'; // switch views first so the match form's parent section is actually visible
    renderApp();
    openMatchEntryForm(match.id);
    return;
  }

  const session = getSessions().find((s) => s.date === date);
  if (session) {
